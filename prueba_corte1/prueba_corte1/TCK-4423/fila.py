...
class Fila:
    ...
    def retirar(self, turno):
        ...
        if self.cabeza.turno == turno:
            #Se cambia la cabeza por el siguiente nodo, eliminando el primero de la fila No se pone el none
            self.cabeza = self.cabeza.siguiente                 # <-- caso 1
            return True
        anterior = self.cabeza
        while anterior.siguiente is not None:
            if anterior.siguiente.turno == turno:
                #Se agrega otro .siguiente para que si no el primero se elimina el nuevo nodo
